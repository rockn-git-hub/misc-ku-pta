...
const { finalizeDocument } = documentsStoreModule.default || documentsStoreModule;
const {
  ADMIN_COOKIE_NAME,
  parseCookies,
...
module.exports = async function handler(req, res) {
  ...
  let doc;
  try {
    doc = await finalizeDocument(docId);
  } catch (error) {
    ...
    if (error.message.startsWith("missing-page:")) {
      sendJson(res, 400, { ok: false, message: `${error.message.split(":")[1]}ページ目が未アップロードです。` });
      return;
    }
  ...
  const viewerPath = buildViewerPath(doc.id);
